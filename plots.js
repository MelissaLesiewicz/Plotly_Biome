...
function buildCharts(sample) {
  ...
  d3.json("samples.json").then((data) => {
	...
	var demographicArray = metadata.filter(sampleObj => sampleObj.id == sample);
    //  5. Create a variable that holds the first sample in the array.
	var result = resultArray[0];
	var demographicInfo = demographicArray[0];
	//console.log(result)
    // 6. Create variables that hold the otu_ids, otu_labels, sample_values, and wash frequency.
	
	//var sortedResults = result.sort((a,b) => a.sample_values - b.sample_values).reverse();
